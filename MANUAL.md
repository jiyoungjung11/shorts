...
> **[정책 변경 2026-05-28]** 이미지 소스를 AI 생성(Vertex AI Imagen 3)에서 **실사 사진**으로 전환.  
> Wikimedia Commons 등 저작권 무료 실제 사진을 `images/` 폴더에 직접 배치 후 `--from images` 이후 단계를 실행할 것.
...
> **[2026-05-28 정책 변경]** 기본 이미지 소스를 **실사 사진**으로 전환.  
> AI 생성 이미지는 실사 구하기 어려운 특수 장면에만 보조적으로 사용.

### 실사 사진 (기본 — 현재 정책)
- 장점: 실제 장소 확실, 시청자 신뢰도 높음, 무료 (CC/Public Domain)
- 단점: 구도·화질이 아쉬울 수 있음, 직접 수집 필요
- **무료 소스**: Wikimedia Commons, Unsplash, Pixabay
- **워크플로**: 사진 수집 → `images/cut_01.png ~ cut_10.png`로 저장 → `--from images` 이후 실행

### 실사 사진 수집 방법
1. Wikimedia Commons에서 검색 후 파일 페이지의 "Original file" URL로 다운로드
2. ffmpeg으로 PNG 변환: `ffmpeg -i input.jpg -y cut_01.png`
3. 이미지 폴더에 `cut_XX.png` 형식으로 저장

### AI 생성 이미지 (보조 — 특수 장면 한정)
- 장점: 시나리오 의도에 딱 맞는 드라마틱한 이미지
- 단점: 실제 장소와 다를 수 있음, API 비용 발생
- 용도: 야경 합성, 항공뷰, 실사 구하기 어려운 장면
